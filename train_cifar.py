...
def parse_args():
    ...
    parser.add_argument('--adaptive_eviction', action=argparse.BooleanOptionalAction, default=False,
                        help='Use adaptive eviction for memory traces.')
    # LSTM specific
...
if __name__=='__main__':
    ...
    testloader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size_test, shuffle=True, drop_last=False)
    ...
    if args.model == 'ctm':
        model = ContinuousThoughtMachine(
            ...
            adaptive_eviction=args.adaptive_eviction,
        ).to(device)
